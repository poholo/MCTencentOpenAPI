...
- (void)showInvalidTokenOrOpenIDMessage{
    UIAlertView *alert = [[[UIAlertView alloc]initWithTitle:@"api调用失败" message:@"参数有误或者token失效，请检查参数或者重新登录" delegate:nil cancelButtonTitle:@"OK" otherButtonTitles:nil, nil] autorelease];
    [alert show];
}

- (IBAction)OnClickCancle
{
    tencentOAuthDemoAppDelegate *delegate = (tencentOAuthDemoAppDelegate *)[[UIApplication sharedApplication] delegate];
    if (nil != delegate)
    {
        UIWindow *window = [delegate window];
        window.rootViewController = (UIViewController *)[delegate viewController];
    }
}
